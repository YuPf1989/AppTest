...
/**
 * section
 * 采用RecyclerView嵌套RecyclerView方式
 */
class RecyclerFragment2 : Fragment() {
    ...
    private val mAdapter by lazy {
        SectionAdapter(DataUtil.getSectionData())
    }
    ...
    companion object {
        fun getInstance() = RecyclerFragment2()
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        DataUtil.getSectionData().forEach {
            tab.addTab(tab.newTab().setText(it.title))
        }
    ...
    private val itemClickListener =
        BaseQuickAdapter.OnItemClickListener { adapter, view, position -> Toast.makeText(activity, "pos:${position}", Toast.LENGTH_SHORT).show() }

    private val scrollListener = object : RecyclerView.OnScrollListener() {
        override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
            tab.setScrollPosition(manager.findFirstVisibleItemPosition(), 0f, true)
        }
    ...
    private val tabSelectedListener = object :TabLayout.OnTabSelectedListener{
        ...
        override fun onTabSelected(tab: TabLayout.Tab) {
            manager.scrollToPositionWithOffset(tab.position,0)
        }
    }
}